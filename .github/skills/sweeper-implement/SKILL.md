...
You are working on a single egamma/sweeper-sandbox issue on behalf of the maintainer who invoked you.
...
- The working directory must be a **egamma/sweeper-sandbox checkout** — `git remote -v` must list
  `egamma/sweeper-sandbox`. If not, stop: "run this from your vscode checkout".
...
```
gh api "repos/egamma/vscodesweeper-state/contents/records/egamma/sweeper-sandbox/items/<issue-number>.md?ref=state" -H "Accept: application/vnd.github.raw"
```
...
```
gh issue view <issue-number> --repo egamma/sweeper-sandbox
```
...
```
gh issue view <issue-number> --repo egamma/sweeper-sandbox --json state,labels,updatedAt
```
...
4. **No open PR already references the issue**
   (`gh search prs --repo egamma/sweeper-sandbox --state open "<issue-number>" --json url,title`,
...
```
gh pr create --repo egamma/sweeper-sandbox --base main --draft --title "<concise title>" --body-file <body file>
```
...
- `Fixes #<issue-number>`
- `Seeded by a VS Code Sweeper review: https://github.com/egamma/vscodesweeper-state/blob/state/records/egamma/sweeper-sandbox/items/<issue-number>.md`
