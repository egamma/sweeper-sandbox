...
test('computeStats counts logical lines and blank-line-delimited paragraphs', () => {
  const cases = [
    { text: '', lines: 1, paragraphs: 0 },
    { text: 'one line', lines: 1, paragraphs: 1 },
    { text: 'one line\n', lines: 2, paragraphs: 1 },
    { text: 'one line\n\n', lines: 3, paragraphs: 1 },
    { text: 'one\n\n\ntwo', lines: 4, paragraphs: 2 },
    { text: 'one\n \t \ntwo', lines: 3, paragraphs: 2 },
    { text: 'first line\nsecond line', lines: 2, paragraphs: 1 },
  ];

  for (const { text, lines, paragraphs } of cases) {
    const stats = computeStats(text);
    assert.equal(stats.lines, lines, JSON.stringify(text));
    assert.equal(stats.paragraphs, paragraphs, JSON.stringify(text));
  }
});
...
test('formatStatsTooltip preserves the hint and adds line and paragraph counts', () => {
