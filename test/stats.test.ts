...
test('computeStats counts logical lines and blank-line-separated paragraphs', () => {
  const cases = [
    { text: '', lines: 0, paragraphs: 0 },
    ...
    { text: 'one\ncontinues', lines: 2, paragraphs: 1 },
  ];

  for (const { text, lines, paragraphs } of cases) {
    const stats = computeStats(text);
    assert.deepEqual({ lines: stats.lines, paragraphs: stats.paragraphs }, { lines, paragraphs });
  }
});
